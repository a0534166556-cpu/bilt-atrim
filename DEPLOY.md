...
| `CJ_SHIPPING_USD` | `4` | משלוח CJ בדולר – **רק** אם `CJ_PRICE_INCLUDES_SHIPPING=true` |
| `CJ_PRICE_INCLUDES_SHIPPING` | `false` | `false` = מחיר מוצר לפי עלות CJ בלבד; משלוח ללקוח בקופה (₪29) |

**נוסחה (ברירת מחדל):** `מחיר CJ ב-$ × 1.30 רווח × שער` → עיגול ל-₪  
דוגמה: מוצר **$1.72** → בערך **₪9** במוצר + **₪29** משלוח בקופה.

אם תרצה לכלול משלוח CJ במחיר המוצר: `CJ_PRICE_INCLUDES_SHIPPING=true`

לעדכון מחירים קיימים: **ניהול → ייבוא CJ → עדכן מחירים לשקלים**.
