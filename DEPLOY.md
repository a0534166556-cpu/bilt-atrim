...
| משתנה | ערך |
|--------|-----|
| `ADMIN_PASSWORD` | סיסמה חזקה (גם למשתמש המנהל) |
| `OWNER_EMAIL` | אימייל מנהל (ברירת מחדל: a0534166556@gmail.com) |
| `OWNER_PHONE` | טלפון החנות (ברירת מחדל: 0508254935) |
...
| `ADMIN_EMAIL` | אימייל לקבלת התראות על הזמנות חדשות (ברירת מחדל: אימייל החנות) |
...
| `CJ_MARKUP_PERCENT` | `25` | אחוז הרווח על העלות הכוללת (מוצר + משלוח) |
| `CJ_SHIPPING_USD` | `4` | משלוח CJ בדולר – משמש כברירת מחדל כש-CJ לא מחזיר עלות משלוח |
| `CJ_PRICE_INCLUDES_SHIPPING` | `true` | המשלוח כלול במחיר המוצר. שים `false` רק אם תרצה לגבות משלוח בנפרד |

**נוסחה (ברירת מחדל):** `(עלות מוצר $ + משלוח CJ $) × 1.25 רווח × שער` → עיגול ל-₪  
המחיר באתר הוא **כולל הכול** – הלקוח מקבל **משלוח חינם** בקופה (אין תוספת ₪).  
דוגמה: מוצר **$2** + משלוח **$3** = $5 × 1.25 × 3.75 = **₪24** סופי ללקוח, רווח 25%.

לעדכון מחירים קיימים: **ניהול → ייבוא CJ → עדכן מחירים לשקלים** (או אוטומטית בכל הפעלת שרת).
